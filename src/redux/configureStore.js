...
SAMPLE_TRACKS.push(...['sampleA', 'sampleB', 'sampleC', '2sample', '3sample'].map((name) => {
  return {
    name: name.replace('sampleA', 'sample A')
      .replace('sampleB', 'sample B')
      .replace('sampleC', 'sample C')
      .replace('2sample', 'samples A + B')
      .replace('3sample', 'samples A + B + C'),
    coverage: `${SAMPLE_DATA_ROOT}/test_data/${name}.bigWig`,
    junctions: `${SAMPLE_DATA_ROOT}/test_data/${name}.SJ.out.tab.bed.gz`,
  }
}))
